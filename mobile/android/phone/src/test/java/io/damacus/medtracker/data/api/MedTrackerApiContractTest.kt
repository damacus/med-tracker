...
class MedTrackerApiContractTest {
    ...
    @Test fun generatedAdapterDecodesCurrentLoginPayloadAndUnknownEnum() = runBlocking {
        ...
        val result = api.login(server.url("/").toString(), LoginRequest("carer@example.test", "password"))
    ...
    @Test fun generatedAdapterUsesContractOperationsForEveryActivePhoneRequest() = runBlocking {
        ...
        assertTrue(api.login(baseUrl, LoginRequest("person@example.test", "password")) is ApiResult.Error)
        ...
        assertEquals(listOf(
            "/api/v1/auth/login", "/api/v1/auth/oidc_exchange", "/api/v1/auth/refresh", "/api/v1/auth/logout",
