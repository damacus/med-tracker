...
RSpec.describe "Medication wizard dose option follow-up" do
  ...
  it "creates the medication, primary dose option, and first schedule from the wizard" do
    ...
    expect do
      ...
      .to(change(PaperTrail::Version.where(item_type: "Medication", event: "create"), :count).by(1))
    ...
    expect(medication.default_schedule_config).to(
      include(
        "schedule_type" => "multiple_daily",
        "frequency" => "Twice daily",
        "times" => %w[08:00 20:00]
    ...
    expect(schedule.schedule_config).to(
      include(
        "schedule_type" => "multiple_daily",
        "frequency" => "Twice daily",
        "times" => %w[08:00 20:00]
  ...
  it "creates the initial dependent adult schedule from the dependent default dose" do
    ...
    medication = Medication.order(:id).last
    dependent_dosage = medication.dosage_records.find_by!(amount: BigDecimal("7.5"), unit: "ml")
    schedule = Schedule.order(:id).last

    expect(schedule).to(
      have_attributes(
        person: dependent,
        source_dosage_option: dependent_dosage,
        dose_amount: BigDecimal("7.5"),
        dose_unit: "ml"
      )
    )
  end
