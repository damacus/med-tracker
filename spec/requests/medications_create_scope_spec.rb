...
RSpec.describe "Medication creation scope" do
  ...
  describe "POST /medications" do
    ...
    it "adds scanned stock to an existing matching medication instead of creating a duplicate" do
      ...
      expect(existing_medication.reload).to(
        have_attributes(
          current_supply: 36,
    ...
    it "fills missing onboarding fields and suggested doses from curated dm+d metadata" do
      ...
      expect(medication.dosage_records.order(:id).pluck(:amount, :unit, :current_supply, :reorder_threshold)).to(
        eq(
          [
