...
RSpec.describe DemoReset::S3StorageCleaner do
  ...
  it 'removes every object from the verified bucket' do
    result = cleaner.call
  ...
  it 'refuses any bucket other than the expected bucket' do
  ...
  it 'refuses any endpoint other than the expected endpoint' do
  ...
  def cleaner(bucket: 'demo-archive', endpoint: 'https://objects.example.test')
    described_class.new(
      ...
      expected_target: { endpoint: 'https://objects.example.test', bucket: 'demo-archive' }
    )
  end
end
