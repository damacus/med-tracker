...
RSpec.describe Reports::MedicationReviewPdf do
  ...
  def pdf_page_texts(pdf)
    objects = pdf_indirect_objects(pdf)
    font_maps = pdf_font_maps(objects)

    objects.values.grep(%r{/Type\s*/Page\b}).map do |page|
      content_references(page).flat_map do |reference|
        decode_page_stream(inflate_object(objects.fetch(reference)), font_maps)
      end.join(' ').unicode_normalize(:nfkd).gsub(/\s+/, ' ').strip
    end
  end

  def pdf_indirect_objects(pdf)
    pdf.scan(/(?:\A|\r?\n)(\d+)\s+\d+\s+obj\r?\n(.*?)\r?\nendobj/m).to_h
  end

  def pdf_font_maps(objects)
    objects.values.each_with_object({}) do |body, maps|
      body.scan(%r{/([A-Za-z0-9]+)\s+(\d+)\s+0\s+R}).each do |name, reference|
        font = objects[reference]
        next unless font&.match?(%r{/Type\s*/Font\b})

        unicode_reference = font[%r{/ToUnicode\s+(\d+)\s+0\s+R}, 1]
        maps[name] = character_map(inflate_object(objects.fetch(unicode_reference))) if unicode_reference
      end
    end
  end

  def content_references(page)
    contents = page[%r{/Contents\s+(\[.*?\]|\d+\s+0\s+R)}m, 1].to_s
    contents.scan(/(\d+)\s+0\s+R/).flatten
  end
  ...
  def decode_page_stream(stream, font_maps)
    ...
    stream.scan(pdf_page_operator_pattern) do |selected_font, direct, _array, array_body|
      ...
      text.concat(operators.filter_map { |operator| decode_operator(operator, font_maps[font_name]) })
    end
    text
  end
