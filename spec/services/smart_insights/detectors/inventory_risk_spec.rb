...
RSpec.describe SmartInsights::Detectors::InventoryRisk do
  ...
  it 'is urgent when the first alert is low stock' do
    ...
    expect(insight.detail).to eq(
      I18n.t('smart_insights.detectors.inventory_risk.detail', medication_name: 'Paracetamol')
    )
  ...
  it 'is a warning when the first alert is not low stock' do
    expect(described_class.new(context_with([alert(low_stock: false, days_left: 5)])).call.first.severity)
      .to eq(:warning)
  end
  ...
  it 'uses the zero-days summary when days_left is not positive' do
    ...
    expect(insight.summary).to eq(
      I18n.t('smart_insights.detectors.inventory_risk.summary_zero', medication_name: 'Paracetamol')
    )
  end
  ...
  it 'uses the countdown summary when days_left is positive' do
    ...
    expect(insight.summary).to eq(
      I18n.t('smart_insights.detectors.inventory_risk.summary', medication_name: 'Paracetamol', count: 3)
    )
  end
end
