  ...
  describe "#call" do
    def count_source_queries(&)
      ...
      subscriber = lambda do |_name, _start, _finish, _id, payload|
        ...
        if sql.include?("FROM \"person_medications\"") &&
            sql.include?("\"person_medications\".\"person_id\"")
          counts[:person_medications] += 1
        end
      end
    ...
    it "includes schedule doses from both the parent and the child" do
      ...
      # child_patient's ibuprofen
      expect(schedules).to(include(schedules(:patient_schedule)))
    end
    ...
    context("when medication is out of stock") do
      let!(:schedule) do
        oos_medication = Medication.create!(
          name: "OOS Med",
          current_supply: 0,
          reorder_threshold: 2,
