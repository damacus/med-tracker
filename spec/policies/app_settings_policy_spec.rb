...
RSpec.describe AppSettingsPolicy, type: :policy do
  it 'permits show? and update? for household managers only' do
    ...
    expect(described_class.new(owner_context, :settings).show?).to be(true)
    expect(described_class.new(administrator.fetch(:context), :settings).update?).to be(true)
