...
RSpec.describe 'PASSKEY-001: WebAuthn/Passkey configuration' do
  ...
  scenario 'Rodauth WebAuthn feature is enabled' do
    # Verify Rodauth has WebAuthn configured
    expect(RodauthMain.configuration.features).to include(:webauthn)
    expect(RodauthMain.configuration.features).to include(:webauthn_login)
  end
end
