...
RSpec.describe DashboardPresenter do
  ...
  describe '#active_schedules' do
    it 'preloads associated medication schedules and person_medications to avoid N+1 queries' do
      # Ensure there is at least one active schedule in the database for the test
      schedules(:john_paracetamol).update!(active: true, start_date: 1.month.ago.to_date, end_date: 1.month.from_now.to_date)

      presenter = presenter_for(admin_user, people_scope: Person.all)
      schedules = presenter.active_schedules

      expect(schedules).to be_present
