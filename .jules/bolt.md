...
## 2026-03-09 - Avoid nil amount issues with Enumerable#sort_by on dosage records
**Learning:** When optimizing database queries using `Enumerable#sort_by` on collections (like `dosage_records`), the `.sort_by { |r| [r.amount, r.id] }` approach can raise an `ArgumentError: comparison of Array with Array failed` if `r.amount` is `nil`. Database `ORDER BY` handles nulls gracefully, but Ruby requires an explicit fallback.
**Action:** When replacing ActiveRecord sorting with Ruby enumerable sorting for models that could potentially have `nil` values in the sorted fields, use a fallback value (e.g., `[r.amount || 0, r.id]`) to prevent runtime exceptions during sorting.
