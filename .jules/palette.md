...
## 2026-06-15 - Component icon-only `aria-label` conventions\n**Learning:** When using RubyUI components or Rails helpers like `m3_button` or `m3_link` for icon-only buttons, use a hyphenated string key (`"aria-label":`) for the accessible name, as these helpers do not auto-convert underscored symbols (`aria_label:`) like native Phlex methods do.\n**Action:** When working with ActionView helpers, always use hyphenated string keys for data and aria attributes to ensure they are properly rendered in the HTML.
