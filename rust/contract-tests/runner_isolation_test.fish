...
for index in (seq (count $targets))
    ...
    if test $index -lt (count $targets)
        ...
        if not contains -- "$targets[$index]" lookup platform retained web_profile web_devices web_json_read web_json_actions push_delivery; and not contains -- "$targets[$next_index]" lookup platform retained web_profile web_devices web_json_read web_json_actions push_delivery
...
test (string join '\n' -- $trace) = (string join '\n' -- $expected_trace)
or begin; diff (string join \n -- $expected_trace | psub) $test_dir/trace >&2; echo 'Full runner did not isolate and refresh every target' >&2; exit 1; end
