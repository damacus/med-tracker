...
function remove_contract_fixture --on-event fish_exit
    if set -q contract_run_dir
        ...
        rtk proxy rm -f "$contract_run_dir/owner"
...
set -g contract_project mtcontract-(string lower (string replace -a - '' (uuidgen)) | string sub -l 16)
string match -rq '^mtcontract-[a-f0-9]{16}$' -- $contract_project
or exit 2
