...
function cleanup_isolation_test --on-event fish_exit
    for pair in "$first_project:$first_dir" "$second_project:$second_dir"
        ...
        if test -f $parts[2]/owner
            rtk task contract:cleanup CONTRACT_PROJECT=$parts[1] CONTRACT_RUN_DIR=$parts[2]
            or begin
...
for pair in "$first_project:$first_dir" "$second_project:$second_dir"
    set -l parts (string split : $pair)
    rtk task contract:prepare-db CONTRACT_PROJECT=$parts[1]
    or exit $status
    rtk task test:server CONTRACT_PROJECT=$parts[1]
    or exit $status
    rtk task --force test:exec CONTRACT_PROJECT=$parts[1] CMD="CONTRACT_FIXTURE_PATH=/app/$parts[2]/fixture.json rails runner scripts/contract_provision.rb"
    or exit $status
end
...
set -l first_port (rtk task test:port CONTRACT_PROJECT=$first_project)
set -l second_port (rtk task test:port CONTRACT_PROJECT=$second_project)
...
for entry in "$first_project:$first_email:$second_email" "$second_project:$second_email:$first_email"
    ...
    set -l counts (rtk task --force test:exec CONTRACT_PROJECT=$fields[1] CMD="rails runner \"puts [Account.where(email: '$fields[2]').count, Account.where(email: '$fields[3]').count, ActiveRecord::Base.connection.select_value('SHOW server_version')].join(':')\"" | string match -r '^[0-9]+:[0-9]+:[0-9].*' | tail -1)
...
rtk task contract:cleanup CONTRACT_PROJECT=$first_project CONTRACT_RUN_DIR=$first_dir
or exit $status
...
set -l remaining (rtk task --force test:exec CONTRACT_PROJECT=$second_project CMD="rails runner \"puts Account.where(email: '$second_email').count\"" | string match -r '^[0-9]+$' | tail -1)
test "$remaining" = 1
...
set -l after_failure (rtk task --force test:exec CONTRACT_PROJECT=$second_project CMD="rails runner \"puts Account.where(email: '$second_email').count\"" | string match -r '^[0-9]+$' | tail -1)
test "$after_failure" = 1
or begin; echo 'Peer fixture disappeared after failure cleanup' >&2; exit 1; end

rtk task contract:cleanup CONTRACT_PROJECT=$second_project CONTRACT_RUN_DIR=$second_dir
or exit $status
