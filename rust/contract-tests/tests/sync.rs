...
fn assert_portable_records(collections: &Value) {
    for category in [
        "people",
        "locations",
        "medications",
        ...
        for row in collections[category].as_array().expect(category) {
            assert!(
...
fn sync_v2_uses_current_view_scope_and_exposes_portable_clinical_relations() {
    ...
    assert_portable_records(collections);
    ...
    assert!(!collections["medication_pause_periods"]
        .as_array()
        .unwrap()
        .is_empty());
