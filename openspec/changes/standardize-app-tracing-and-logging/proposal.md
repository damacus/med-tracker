...
- Build an exhaustive registry of application-owned event publishers, subscribers, and direct logger call sites, and require every custom event to have a production-visible, privacy-safe operational disposition.
...
- Make event dispatch, subscriber outcomes, transaction commit or rollback, retries, and subscriber failures observable without reporting an uncommitted side effect as successful.
...
- Affects production logger and Lograge configuration, OpenTelemetry correlation, every application-owned Active Support notification publisher and subscriber, direct application logger calls, background-job context, and safety-critical services and jobs.
