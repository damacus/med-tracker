...
API application tokens, including MCP tokens, gain a centrally configured positive maximum age with an agreed default of 12 calendar months from issuance. Store an expiry for each issued token no later than issuance plus the configured maximum, expose expiry in token-management responses/UI, and reject expired tokens across all accepting entry points. Usage must not slide or refresh expiry. Shorter requested expiry is allowed; longer or unlimited values are rejected. Apply any reduced instance maximum as an upper bound based on issuance, and never revive an expired token by increasing the setting. Keep revocation and membership restrictions independent of expiry. Verify calendar boundaries, including leap-day issuance, rather than treating twelve months as twelve fixed 30-day periods.
