...
All initial authentication and fresh verification go through Rodauth. When an existing protected action lacks acceptable evidence, the phone opens the instance's Rodauth verification flow; Rodauth handles local verification or upstream delegation and returns through the registered mobile callback. Bind the verification attempt to the initiating account and mobile session, state and PKCE; a different browser account must not upgrade the original account's session. Cancellation leaves the action unperformed. After success, recheck the action's current household/person authority and preserve idempotency before resuming; never execute a write as part of an unauthenticated callback.

Current-code inventory: API freshness gates cover creating/revoking/resending invitations, updating/revoking memberships, creating/revoking person access grants and app tokens, and updating household settings. API evidence currently requires an OIDC-marked ApiSession timestamp within 15 minutes. Web platform writes are gated; most web household-admin pages depend on HOSTED_ADMIN_MFA_REQUIRED, with an explicit gate on ambiguous-person-grant review. The web OIDC flag short-circuits freshness checks and the local web timestamp is renewed on accepted privileged actions. Preserve the policy scope and flag semantics while explicitly testing evidence differences; do not infer a new universal MFA requirement from these implementation inconsistencies.
