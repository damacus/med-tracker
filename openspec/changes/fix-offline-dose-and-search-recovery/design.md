...
3. Select stock and insert the queued take in one IndexedDB read/write transaction, including across browser documents. Guard repeated clicks per button so actions for different medicines remain independent. Match the server's countable-unit and volume consumption rules. Close read connections after requests finish. Offer retry only for pending sync failures.
...
- Server eligibility checks can add snapshot queries → batch stock, timing history, related sources and forecast associations at the controller/service boundary; cache recording permission per person and source type within the request. Test query growth and preserve the ordinary resolver's authorisation and timing decisions.
