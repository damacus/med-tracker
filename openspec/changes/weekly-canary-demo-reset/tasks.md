...
- [x] 1.2 Run `task test:preflight` and record the current focused seed/reset test baseline before coding.
- [x] 1.3 Write failing RSpec and system coverage proving demo mode is off by default and requires explicit configuration. Prove authenticated pages show an accessible disposable-data and weekly-reset notice without changing authorization or medication behavior.
- [x] 1.4 Add the application-owned demo-mode boundary and shared-layout notice, then make the focused configuration and system specs pass.
...
- [x] 2.2 Add the canary-only reset preflight and PHI-safe stage/result reporting, then make the safety specs pass.
...
- [x] 2.4 Add advisory locking plus transactional table replacement and baseline loading, then make the database reset specs pass.
...
- [x] 2.6 Add verified canary storage cleanup and post-reset invariant checks, then make the storage/reset specs pass.
...
- [ ] 4.3 Add the reset CronJob controller to the canary HelmRelease using the exact MedTracker image SHA and demo-mode configuration, owner-capable canary database role, isolated canary S3 bucket, and bounded runtime/retries/history. Use a reset-only process without an application storage mount. Quiesce user mutations and queue writers until strict reset verification succeeds.
- [ ] 4.4 Reduce canary CNPG to one clean `initdb` instance with ordinary storage and monitoring. Disable superuser access. Remove `externalClusters`, every physical or logical backup resource, the Barman WAL-archiver plugin, ScheduledBackup, ObjectStore, and canary RustFS backup credential/bootstrap resources.
