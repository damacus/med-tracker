...
function generate_kotlin_package --no-scope-shadowing
    ...
    ruby "$script_dir/augment_clearable_fields.rb" "$contract" "$temporary_output" kotlin
end
...
if not generate_kotlin_package output
    exit 1
end
...
if test "$argv[1]" = determinism
    if not generate_kotlin_package repeat
        exit 1
    end
