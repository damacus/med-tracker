  ...
  module Rodauth
    class UnlockAccountRequest < Views::Rodauth::Base
      ...
      def render_unlock_request_form
        render(
          RubyUI::Form.new(
            action: rodauth.unlock_account_request_path,
            method: :post,
            class: "space-y-6",
            data_turbo: "false"
          )
      ...
      def render_other_options
        div(class: "space-y-4 border-t border-outline-variant/30 pt-8") do
          h3(class: "text-[10px] font-black uppercase tracking-widest text-on-surface-variant") {
            t("rodauth.views.reset_password_request.other_options")
          }
          div(class: "flex flex-col gap-2 items-start") do
            m3_link(href: rodauth.login_path, variant: :text, size: :sm, class: "font-bold h-auto p-0 underline") {
            ...
            m3_link(
              ...
              variant: :text,
              size: :sm,
              class: "font-bold h-auto p-0 underline"
            ) do
