...
module Views
  module Rodauth
    class TwoFactorManage < Views::Base
      ...
      def header_section
        div(class: 'mx-auto max-w-xl text-center space-y-3') do
          h1(class: 'text-3xl font-bold tracking-tight text-slate-800 sm:text-4xl') do
            if view_context.rodauth.two_factor_authentication_setup?
      ...
      def render_card_content
        render RubyUI::CardContent.new(class: 'space-y-4 p-6 sm:p-8') do
          ...
          render_totp_option
          render_recovery_codes_option if view_context.rodauth.two_factor_authentication_setup?
        end
      end
      ...
      def render_webauthn_option
        render_auth_method_card({
                                  ...
                                  icon: :passkey,
                                  setup_path: view_context.rodauth.webauthn_setup_path,
                                  setup_text: 'Set up passkey',
                                  enabled: webauthn_enabled?,
                                  manage_path: webauthn_enabled? ? view_context.rodauth.webauthn_remove_path : nil,
      ...
      def render_totp_option
        render_auth_method_card({
                                  ...
                                  icon: :totp,
                                  setup_path: view_context.rodauth.otp_setup_path,
                                  setup_text: 'Set up authenticator',
                                  enabled: totp_enabled?,
                                  manage_path: totp_enabled? ? view_context.rodauth.otp_disable_path : nil,
      ...
      def render_recovery_codes_option
        render_auth_method_card({
                                  ...
                                  icon: :recovery,
                                  setup_path: view_context.rodauth.recovery_codes_path,
      ...
      def render_passkey_icon(icon_class)
        svg(class: "w-6 h-6 #{icon_class}", fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', stroke_width: '2') do |s|
          s.path(stroke_linecap: 'round', stroke_linejoin: 'round', d: 'M12 11c0 3.517-1.009 6.799-2.753 9.571m-3.44-2.04l.054-.09A13.916 13.916 0 008 11a4 4 0 118 0c0 1.017-.07 2.019-.203 3m-2.118 6.844A21.88 21.88 0 0015.171 17m3.839 1.132c.645-2.266.99-4.659.99-7.132A8 8 0 008 4.07M3 15.364c.64-1.319 1-2.8 1-4.364 0-1.457.39-2.823 1.07-4')
        end
      end

      def render_totp_icon(icon_class)
        svg(class: "w-6 h-6 #{icon_class}", fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', stroke_width: '2') do |s|
          s.path(stroke_linecap: 'round', stroke_linejoin: 'round', d: 'M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z')
        end
      end

      def render_recovery_icon(icon_class)
        svg(class: "w-6 h-6 #{icon_class}", fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', stroke_width: '2') do |s|
          s.path(stroke_linecap: 'round', stroke_linejoin: 'round', d: 'M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z')
        end
      end
      ...
      def webauthn_enabled?
        view_context.rodauth.uses_webauthn_authentication?
      rescue StandardError
        false
      end

      def totp_enabled?
        view_context.rodauth.otp_exists?
      rescue StandardError
        false
      end

      def recovery_codes_enabled?
        view_context.rodauth.recovery_codes_exist?
      rescue StandardError
        false
      end
    end
