  ...
  module Rodauth
    class WebauthnSetup < Views::Rodauth::Base
        ...
        div(class: "space-y-3 rounded-2xl border border-outline-variant/30 bg-secondary-container/30 p-5") do
          div(class: "flex items-start gap-3") do
            ...
            div(class: "space-y-1") do
              m3_heading(variant: :title_small, class: "font-bold text-foreground") {
                t("rodauth.views.webauthn_setup.info_title")
              }
      ...
      def hidden_webauthn_fields(credential_options)
        ...
        input(
          type: "hidden",
          name: view_context.rodauth.webauthn_setup_challenge_param,
          value: credential_options.challenge
        )
        input(
          type: "hidden",
          name: view_context.rodauth.webauthn_setup_challenge_hmac_param,
          value: view_context.rodauth.compute_hmac(credential_options.challenge)
        )
      end
      ...
      def password_field
        render_m3_form_field(
          ...
          p(class: "mt-1 px-1 text-xs text-on-surface-variant font-medium") {
            t("rodauth.views.webauthn_setup.current_password_hint")
          }
        end
      end
      ...
      def render_webauthn_script
        script(
          src: "#{view_context.rodauth.webauthn_js_host}#{view_context.rodauth.webauthn_setup_js_path}",
