    ...
    class Index < Views::Base
      ...
      def render_header
        div(class: "mb-12 flex flex-col gap-6 md:flex-row md:items-end md:justify-between") do
          div(class: "space-y-2 text-center md:text-left") do
            ...
            m3_heading(level: 1, size: "8", class: "font-extrabold tracking-tight text-foreground") {
          ...
          form(
            action: view_context.reports_path,
            method: :get,
            class: "flex items-end gap-3 rounded-[1.5rem] border border-border/70 bg-popover p-4 shadow-elevation-1"
          ) do
            div(class: "flex flex-col gap-1") do
              label(for: "start_date", class: "text-xs font-semibold uppercase tracking-wider text-on-surface-variant") {
                t("reports.index.start_date_label")
              }
              input(
                type: "date",
                name: "start_date",
                id: "start_date",
                value: @start_date,
                class: "form-input rounded-lg border-border bg-background text-sm text-foreground focus:border-primary focus:ring-primary"
              )
            end

            div(class: "flex flex-col gap-1") do
              label(for: "end_date", class: "text-xs font-semibold uppercase tracking-wider text-on-surface-variant") {
                t("reports.index.end_date_label")
              }
              input(
                type: "date",
                name: "end_date",
            ...
            m3_button(
              ...
              'aria-label': t("reports.index.apply_filters_aria_label")
            ) do
          ...
          div(class: "relative p-8 sm:p-12") do
            div(class: "relative z-10 grid grid-cols-1 md:grid-cols-3 gap-8 text-center") do
              summary_stat(
                ...
                "#{overall_compliance}%",
                overall_compliance >= 90 ? t("reports.index.summary.excellent") : t(
                  "reports.index.summary.needs_attention"
                )
              )
              summary_stat(
                t("reports.index.summary.total_doses_logged"),
                "#{total_actual}/#{total_expected}",
                total_actual >= total_expected ? t("reports.index.summary.on_track") : t(
                  "reports.index.summary.missed_doses"
                )
              )
      ...
      def summary_stat(label, value, subtext)
        div(class: "space-y-1") do
          ...
          m3_text(
            size: "1",
            class: "inline-block rounded-full bg-primary-foreground/20 px-2 py-0.5 font-bold text-primary-foreground"
          ) { subtext }
        end
      end
        ...
        div(class: "flex-1 flex flex-col items-center gap-4 h-full justify-end group") do
          ...
          div(class: "relative w-full h-full flex flex-col justify-end") do
            ...
            div(
              ...
              div(
                class: "absolute -top-10 left-1/2 -translate-x-1/2 rounded bg-foreground px-2 py-1 text-[10px] font-bold whitespace-nowrap text-background opacity-0 transition-opacity group-hover:opacity-100"
              ) do
          ...
          # Day Label
          m3_text(size: "1", weight: "bold", class: "text-on-surface-variant uppercase tracking-tighter font-black") {
            day[:day_name]
          }
        end
      end
      ...
      def render_inventory_alert
        ...
        render_insight_card(
          InsightCard.new(
            ...
            value: days_left <= 0 ? t("reports.index.inventory_alert.out_of_stock") : t(
              "reports.index.inventory_alert.refill_pending"
            ),
      ...
      def render_insight_card(card)
        m3_card(
          class: "space-y-4 border border-border/70 bg-card p-8 shadow-elevation-1 transition-transform hover:scale-[1.02] hover:shadow-elevation-2"
        ) do
          div(class: "flex items-center gap-4") do
            div(
              class: "flex h-12 w-12 items-center justify-center rounded-2xl #{card.icon_background_class} #{card.text_color}"
            ) do
            ...
            div do
              ...
              m3_text(size: "1", weight: "bold", class: "uppercase tracking-widest opacity-50") {
                t("reports.index.actionable_insight")
              }
            end
          end
