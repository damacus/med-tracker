  ...
  module Profiles
    class AccountSecurityCard < Components::Base
      ...
      def render_email_change_sheet
        div(
          class: "flex items-start justify-between rounded-shape-lg border border-border/70 bg-popover p-4 shadow-elevation-1 transition-colors hover:border-primary/20 hover:bg-tertiary-container/40"
        ) do
          div(class: "flex-1") do
            h3(class: "text-sm font-medium text-foreground") { t("profiles.account_security.change_email_title") }
            p(class: "mt-1 text-sm text-on-surface-variant") {
              t("profiles.account_security.change_email_description")
            }
          end
      ...
      def render_password_change_sheet
        div(
          ...
          div(class: "flex-1") do
            ...
            p(class: "mt-1 text-sm text-on-surface-variant") {
              t("profiles.account_security.change_password_description")
            }
          end
