...
module Views
  module Profiles
    class CloseAccountDialog < Views::Base
      ...
      def view_template
        div(
          class: "flex items-start justify-between rounded-shape-lg border border-destructive/20 bg-popover p-4 shadow-elevation-1 transition-colors hover:border-destructive/30 hover:bg-tertiary-container/40"
        ) do
          render_account_info
          render_alert_dialog
        end
      ...
      def render_close_account_form
        render(RubyUI::Form.new(action: view_context.rodauth.close_account_path, method: :post, class: "space-y-3")) do
          ...
          m3_button(type: :submit, variant: :destructive, class: "w-full") {
      ...
      def render_close_account_password_field
        render(RubyUI::FormField.new) do
          render(
            RubyUI::FormFieldLabel.new(for: "close-account-password") {
              t("rodauth.views.change_login.password_label")
            }
          )
          m3_input(
            type: :password,
