      ...
      if @person.update(person_params)
            ...
            render(
              turbo_stream: [
                ...
                turbo_stream.replace(
                  "main-content",
                  Views::Profiles::Show.new(person: @person.reload, account: @account, user: current_user)
                )
              ]
            )
          end
        ...
        respond_to do |format|
          format.html {
            redirect_to(
              profile_path,
              alert: t("profiles.profile_update_failed", errors: @person.errors.full_messages.join(", "))
            )
          }
          format.turbo_stream do
      ...
      if @account.update(account_params)
          ...
          format.turbo_stream do
            ...
            render(
              turbo_stream: [
                ...
                turbo_stream.replace(
                  "main-content",
                  Views::Profiles::Show.new(person: @person, account: @account.reload, user: current_user)
                )
              ]
            )
          end
        ...
        respond_to do |format|
          format.html {
            redirect_to(
              profile_path,
              alert: t("profiles.email_update_failed", errors: @account.errors.full_messages.join(", "))
            )
          }
          format.turbo_stream do
  ...
  def experiments
    ...
    if current_user.update(wizard_variant: variant)
      ...
      respond_to do |format|
        ...
        format.html { redirect_to(profile_path, alert: "Could not save preference.") }
      end
    end
  end
