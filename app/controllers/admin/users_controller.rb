    ...
    def update
      ...
      respond_to do |format|
        if @user.update(user_params)
          ...
          format.html do
            render(
              Components::Admin::Users::FormView.new(user: @user, locations: load_locations),
      ...
      respond_to do |format|
        if @user == current_user
          ...
          format.turbo_stream do
            ...
            render(
              turbo_stream: turbo_stream.update("flash", Components::Layouts::Flash.new(alert: flash[:alert])),
              status: :unprocessable_content
            )
          end
        else
