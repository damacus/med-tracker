...
module RubyUI
  class CalendarDays < Base
    BASE_CLASS = "inline-flex items-center justify-center rounded-md text-sm ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 h-8 w-8 min-h-[32px] min-w-[32px] p-0 font-normal aria-selected:opacity-100"
    ...
    def date_template(target, &)
      template(data: {ruby_ui__calendar_target: target}) do
        td(
          class: "relative p-0 text-center text-sm focus-within:relative focus-within:z-20 [&:has([aria-selected])]:bg-tertiary-container [&:has([aria-selected])]:rounded-md",
          role: "presentation",
          &
        )
      end
