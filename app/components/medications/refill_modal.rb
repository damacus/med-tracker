# frozen_string_literal: true

module Components
  module Medications
    class RefillModal < Components::Base
      include Phlex::Rails::Helpers::FormWith

      attr_reader :medication, :button_variant, :button_class, :button_size, :quantity,
                  :restock_date, :icon_only, :button_label, :icon

      # rubocop:disable Metrics/ParameterLists
      def initialize(medication:, button_variant: :outlined, button_class: '', button_size: :md,
                     quantity: nil, restock_date: nil, icon_only: false, button_label: nil, icon: nil)
        @medication = medication
        @button_variant = button_variant
        @button_class = button_class
        @button_size = button_size
        @quantity = quantity
        @restock_date = restock_date
        @icon_only = icon_only
        @button_label = button_label
        @icon = icon
        super()
      end
      # rubocop:enable Metrics/ParameterLists

      def view_template
        Dialog do
          DialogTrigger do
            m3_button(variant: button_variant, size: button_size, class: button_class, **(icon_only ? { "aria-label": t('medications.refill_modal.refill_inventory') } : {})) do
              if icon_only
                render Icons::RefreshCw.new(size: 16, aria_hidden: 'true')
              else
                if icon
                  icon_class = button_variant == :outlined ? 'mr-2 text-primary' : 'mr-2'
                  render icon.new(size: 18, class: icon_class, aria_hidden: 'true')
                end
                span { button_label || t('medications.refill_modal.refill_inventory') }
              end
            end
          end

          DialogContent(size: :md) do
            DialogHeader do
              DialogTitle { t('medications.refill_modal.title', medication: medication.display_name) }
              DialogDescription { t('medications.refill_modal.description') }
            end

            DialogMiddle do
              render_form
            end
          end
        end
      end

      private

      def render_form
        form_with(url: refill_medication_path(medication), method: :patch, class: 'space-y-4') do
          div(class: 'space-y-2') do
            label(for: 'refill_quantity', class: 'text-sm font-medium') { t('medications.refill_modal.quantity') }
            input(
              id: 'refill_quantity',
              type: 'number',
              name: 'refill[quantity]',
              required: true,
              value: quantity_value,
              min: '0.01',
              step: '0.01',
              class: 'w-full rounded-shape-sm border border-outline bg-background px-3 py-2 text-sm ' \
                     'focus:ring-2 focus:ring-primary/20 transition-all'
            )
          end

          div(class: 'space-y-2') do
            label(for: 'refill_restock_date', class: 'text-sm font-medium') do
              t('medications.refill_modal.restock_date')
            end
            input(
              id: 'refill_restock_date',
              type: 'date',
              name: 'refill[restock_date]',
              required: true,
              value: restock_date_value,
              class: 'w-full rounded-shape-sm border border-outline bg-background px-3 py-2 text-sm ' \
                     'focus:ring-2 focus:ring-primary/20 transition-all'
            )
          end

          div(class: 'flex justify-end gap-3 pt-2') do
            m3_button(type: :submit, variant: :filled) { t('medications.refill_button') }
          end
        end
      end

      def restock_date_value
        restock_date.presence || Date.current.to_s
      end

      def quantity_value
        return if quantity.blank?

        MedicationStockQuantityFormatter.format(quantity)
      end
    end
  end
end
