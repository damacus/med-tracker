    ...
    class ShowView < Components::Base
      ...
      def view_template
        div(id: "medication_show_#{medication.id}", class: "container mx-auto px-4 py-12 max-w-6xl space-y-12") do
          ...
          div(
            class: "grid grid-cols-1 lg:grid-cols-3 gap-12 #{header_content_offset_class}",
            data: {testid: "medication-content"}
          ) do
      ...
      def render_reorder_actions
        config = if medication.reorder_status.nil?
          {
            path: mark_as_ordered_medication_path(medication),
            label: t("medications.show.mark_as_ordered"),
            icon: Icons::Clock
          }
        elsif medication.reorder_ordered?
          {
            path: mark_as_received_medication_path(medication),
            label: t("medications.show.mark_as_received"),
            icon: Icons::Check
          }
        end
      ...
      def render_adjust_inventory_modal
        render(
          Components::Medications::AdjustInventoryModal.new(
            medication: medication,
            button_variant: :filled,
            button_size: :lg,
