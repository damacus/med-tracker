    ...
    class FormFields < Components::Base
      ...
      def render_workflow_step(step_number, title:, description:, &)
        div(
          ...
          data: {person_medication_form_target: "stepPanel", step: step_number}
        ) do
        ...
        div(class: selection_summary_layout_classes(show_dose)) do
          div(class: "rounded-shape-xl border border-border/60 bg-popover px-4 py-3 shadow-elevation-1") do
            ...
            m3_text(
              size: "3",
              weight: "semibold",
              data: {person_medication_form_target: "selectedMedicationName"}
            ) do
      ...
      def render_dose_field
        FormField do
          ...
          select(
            ...
            data: {
              person_medication_form_target: "doseOptionInput",
      ...
      def initial_step
        if person_medication.errors.any? &&
            workflow_error_attributes.none? do |key|
