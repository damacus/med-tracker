        ...
        FormField do
          render(
            RubyUI::FormFieldLabel
              ...
              ) { t("schedules.workflow.schedule_type_label") }
          )
          select(
            name: "schedule_type",
            id: "schedule_type",
            class: "w-full rounded-md border border-outline bg-surface-container-lowest px-3 py-4 text-sm"
          ) do
            option(value: "", selected: schedule_type.blank?) { t("schedules.workflow.schedule_type_placeholder") }
            option(value: "otc", selected: schedule_type == "otc") {
              t("schedules.workflow.schedule_type_options.otc")
            }
          ...
          render(
            RubyUI::FormFieldLabel
              .new(
                for: "person_id",
                class: "text-[10px] font-black uppercase tracking-widest text-on-surface-variant ml-1"
              ) { t("schedules.workflow.person_label") }
          )
          select(
            name: "person_id",
            id: "person_id",
          ...
          render(
            RubyUI::FormFieldLabel
              .new(
                for: "medication_id",
                class: "text-[10px] font-black uppercase tracking-widest text-on-surface-variant ml-1"
              ) { t("schedules.workflow.medication_label") }
          )
          select(
            name: "medication_id",
            id: "medication_id",
          ...
          render(
            RubyUI::FormFieldLabel
              .new(
                for: "frequency",
                class: "text-[10px] font-black uppercase tracking-widest text-on-surface-variant ml-1"
              ) { t("schedules.workflow.frequency_label") }
          )
          m3_input(
            type: :text,
            name: "frequency",
