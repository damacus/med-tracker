      ...
      def view_template
        div(class: "container mx-auto px-4 py-12 max-w-6xl", data: {testid: "active-schedules-list"}) do
          ...
          if schedules.to_a.any?
                  ...
                  schedules.each do |schedule|
                    tr do
                      td(class: "px-6 py-5 font-bold text-foreground") do
                        m3_link(
                          href: person_path(schedule.person),
                          variant: :text,
                          size: :sm,
                          class: "p-0 h-auto no-underline hover:text-primary"
                        ) do
            ...
            m3_card(
              variant: :elevated,
              class: "p-16 text-center rounded-[2.5rem] border-dashed border-2 border-outline-variant/50"
            ) do
