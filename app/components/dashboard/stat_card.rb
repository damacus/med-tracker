    ...
    class StatCard < Components::Base
      ...
      def view_template
        Card(
          ...
          CardContent(class: 'p-5 relative overflow-hidden') do
            # Subtle background icon for depth
            div(
              class: 'absolute -right-2 -bottom-2 opacity-5 group-hover:scale-110 transition-transform ' \
            ...
            div(class: 'relative z-10 space-y-1') do
              div(class: 'flex items-center justify-between mb-2') do
                Text(
                  size: '1', weight: 'muted',
                  class: 'uppercase font-black tracking-widest group-hover:text-primary transition-colors'
                ) do
                  title
                end
                div(class: "p-2 rounded-lg #{icon_bg_class} #{value_color_class} transition-colors") do
                  render_icon(size: 16)
                end
              end
