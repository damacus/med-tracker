  ...
  module Dashboard
    class TimelineItem < Components::Base
      ...
      def render_action_button
        ...
        form_with(url: path, method: :post, data: { controller: 'optimistic-take', action: 'submit->optimistic-take#submit' }) do |f|
          input(type: :hidden, name: 'authenticity_token', value: view_context.form_authenticity_token)
          f.hidden_field :amount_ml, value: amount
          f.button(type: :submit, class: 'inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input bg-background hover:bg-accent hover:text-accent-foreground h-9 px-3', data: { optimistic_take_target: 'button' }) do
