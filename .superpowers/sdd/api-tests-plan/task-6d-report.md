...
The replay check previously compared counts for seven collections and omitted `person_medications`. A repeat import could have added another direct assignment without failing the assertion. The contract now compares the entire public `records` snapshot before and after replay. This includes all eight portable v1 collections and fails on changed values, links, portable IDs, or additional rows. Rails returns 201 for the exact replay and the stronger focused contract passes 2/2. The matrix now describes the observed public snapshot equality rather than only stable record counts. This remains HTTP-visible evidence; the disposable PostgreSQL migration rehearsal must prove physical table and rollback invariants.

The full Rails contract run was red twice after this assertion change: `schedules` received 429 instead of 200 from an audit read, and `sync` audit reads received 429 or lacked the expected success `data`. The portability suite passed 2/2 in both full runs. In fresh disposable runs, `schedules` passed 11 with 4 ignored and `sync` passed 15 with 1 ignored. The full runner shares one Rails server and client IP across all suites; `config/initializers/rack_attack.rb` limits API audit-log GET requests to 100 per IP per minute. Splitting the full runner into groups with fresh disposable server/fixture instances would isolate that rate-limit budget while retaining production throttle behaviour. The full suite remains a failed gate for this round; its earlier 143-pass result predates this replay assertion.
